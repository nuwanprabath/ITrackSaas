...
namespace ITrackERP.Tests
{
    public abstract class ITrackERPTestBase : AbpIntegratedTestBase<ITrackERPTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ITrackERPTestBase()
        {
        ...
        protected void UsingDbContext(Action<ITrackERPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ITrackERPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ITrackERPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ITrackERPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ITrackERPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ITrackERPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ITrackERPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ITrackERPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ITrackERPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ITrackERPDbContext, Task<T>> func)
        {
            T result;
